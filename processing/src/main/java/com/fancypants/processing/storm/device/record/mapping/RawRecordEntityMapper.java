...
public class RawRecordEntityMapper implements
		EntityMapper<RawRecordEntity, TridentTuple> {

	@Override
	public RawRecordEntity convert(TridentTuple tuple) {
		// extract the id attributes
		RawRecordId id = new RawRecordId(tuple.getStringByField(RawRecordEntity.DEVICE_ATTRIBUTE), 
				UUID.fromString(tuple.getStringByField(RawRecordEntity.UUID_ATTRIBUTE)));
		// extract the rest of the fixed attributes
		Date timestamp = new Date(tuple.getLongByField(RawRecordEntity.TIMESTAMP_ATTRIBUTE));
		float duration = tuple.getLongByField(RawRecordEntity.DURATION_ATTRIBUTE);
		...
		for (int i = 1; i <= DeviceEntity.MAX_CIRCUITS; i++) {
			...
			RawMeasurementEntity measurement = new RawMeasurementEntity(i, tuple.getFloatByField(voltageField), tuple.getFloatByField(amperageField));
			measurements.put(i, measurement);
		}
		// create the entity
		RawRecordEntity entity = new RawRecordEntity(id, timestamp, duration, measurements);
		return entity;
	}
