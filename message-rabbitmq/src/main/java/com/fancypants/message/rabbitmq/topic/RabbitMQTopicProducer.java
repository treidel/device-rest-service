...
public class RabbitMQTopicProducer implements TopicProducer {
	private static final Logger LOG = LoggerFactory
			.getLogger(RabbitMQTopicProducer.class);
	...
	public void sendMessage(String message) throws AbstractMessageException {
		...
		try {
			...
			LOG.warn("unable to send message to connection="
					+ channel.getConnection() + " on exchange=" + exchange);
			throw new RabbitMQException(e);
		}
		LOG.trace("RabbitMQTopicProducer.sendMessage exit");
	}

	@Override
	public void close() {
		LOG.trace("RabbitMQTopicProducer.close enter");
		// close the channel
		try {
			channel.close();
		} catch (IOException e) {
		...
		LOG.trace("RabbitMQTopicProducer.close exit");
	}
}
