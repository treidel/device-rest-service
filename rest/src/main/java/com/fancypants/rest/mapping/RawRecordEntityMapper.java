...
@Component
public class RawRecordEntityMapper implements EntityMapper<RawRecordEntity, Pair<DeviceEntity, RawRecord>> {

	@Override
	public RawRecordEntity convert(Pair<DeviceEntity, RawRecord> entity) throws DataValidationException {
		// extract the inputs
		DeviceEntity deviceEntity = entity.getLeft();
		RawRecord record = entity.getRight();
		// make sure we have data for each circuit
		if (deviceEntity.getCircuits().size() != record.getMeasurements().size()) {
			throw new DataValidationException("mismatched circuit count " + deviceEntity.getCircuits().size() + "!="
		...
		for (RawMeasurement measurement : record.getMeasurements()) {
			...
			if (null != circuitEntity) {
				RawMeasurementEntity measurementEntity = new RawMeasurementEntity(circuitEntity.getIndex(),
						measurement.getVoltageInVolts(), measurement.getAmperageInAmps());
				circuits.put(circuitEntity.getIndex(), measurementEntity);
			} else {
				throw new DataValidationException("invalid circuit=" + measurement.getCircuit());
			}
		}
		// create the entity
		RawRecordEntity recordEntity = new RawRecordEntity(new RawRecordId(deviceEntity.getDevice(), record.getUUID()),
